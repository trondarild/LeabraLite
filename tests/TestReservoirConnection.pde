class TestReservoirConnection {
    String modelname = "Test reservoir connection";
    ...
    TestReservoirConnection () {
        ...
        adeno_con_spec = new ConnectionSpec(ffexcite_spec);
        adeno_con_spec.type = ADENOSINE;
        
        // reservoirs
        // LeakyIntegratorSpec li_spec = new LeakyIntegratorSpec;
        // li_spec.type = ADENOSINE; // not needed?
        adeno_res = new Reservoir(1, ADENOSINE, "Adeno reservoir");
        ...
        hidden_res_conn = new ReservoirConnection(hidden_layer, adeno_res, adeno_con_spec);

        // network
        network_spec.do_reset = false; // since dont use learning, avoid resetting every quarter

        Layer[] layers = {input_layer, hidden_layer};
        Reservoir[] resrvs = {adeno_res};
        Connection[] conns = {IH_conn, hidden_res_conn};


        netw = new Network(network_spec, layers, conns, resrvs);
        netw.build();
    }
    ...
    void draw() {
        ...
        float[][] r_viz = {adeno_res.getOutput()};
        ...
        pushMatrix();
            ...
            pushMatrix();
            translate(10, 50);
            barchart_array(multiply(0.10, adeno_res.getOutput()), adeno_res.name);
            popMatrix();
